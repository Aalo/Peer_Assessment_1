...
read_data<-function(){
    ...
    act<-read.csv(con,header=T,colClasses=c("numeric","character","numeric"))
...
paste("The interval",val$interval,"has the maximum average of steps",round(val$steps,0))
```
...
calc_steps_per_interval <- function(act) {
    steps_perin <- aggregate(act$steps, by = list(interval = act$interval), FUN = mean, 
        na.rm = T)
...
new_act<-data.frame(steps=impute_means(act,steps_per_interval),date=act$date,interval=act$interval)
```
